...
<body>
  ...
  <div class="modal-backdrop" id="link-modal" aria-hidden="true">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="link-modal-title">
      ...
      <p id="link-modal-desc">Enter the HTML file name under projects/ (example: agro_brdf or forest_topo).</p>
      <div class="field">
        <label for="link-input">HTML file name</label>
        <input id="link-input" type="text" placeholder="forest_topo or agro_brdf.html" />
      </div>
  ...
  <script>
    (function () {
      ...
      function normalizePageName(input) {
        ...
        name = name.replace(/^projects\//i, "");
        if (!/\.html?$/i.test(name)) name += ".html";
        if (name.indexOf("/") >= 0) return "";
        return "projects/" + name;
      }

      function githubContentsUrl(fileName) {
        return "https://api.github.com/repos/PNU-qureos-lab/LabLog/contents/" +
      ...
      function pageExists(fileName) {
        return fetch(fileName, { method: "GET", cache: "no-store" })
          .then(function (res) {
            ...
            return fetch(githubContentsUrl(fileName))
      ...
      function openLinkModal(idx, preset) {
        ...
        var shown = preset || row.pageLink || "";
        shown = String(shown).replace(/^projects\//i, "");
        input.value = shown;
        msg.textContent = "";
        ...
        var out = {
          ...
          projects: asArray(data.projects).map(function (r) {
            ...
            return {
              ...
              pageLink: normalizePageName(r.pageLink != null ? String(r.pageLink) : "")
            };
          }),
