    ...
    <section class="panel" id="pane-graph">
      ...
      <div class="graph-layout">
        ...
        <aside class="graph-toolbar">
          ...
          <div class="hint">Drag from output (right) to input (left) to link. Click an edge or its X to remove. Del deletes selected edge.</div>
        </aside>
      </div>
    </section>
  </div>
  ...
  <script>
    (function () {
      ...
      function readDomIntoState() {
        ...
        document.querySelectorAll(".gnode").forEach(function (el) {
          ...
          if (titleEl && node.type === "task") {
            node.title = plainText(getRichHtml(titleEl));
          }
      ...
      function nodeCenter(node, side) {
        ...
        var cy = y + NODE_H / 2;
        if (side === "out") return { x: x + NODE_W, y: cy };
        return { x: x, y: cy };
      }
      ...
      function renderEdges() {
        ...
        state.edges.forEach(function (edge) {
          ...
          if (!from || !to) return;
          var a = nodeCenter(from, "out");
          var b = nodeCenter(to, "in");
        ...
        if (linkDrag) {
          ...
          temp.setAttribute("d", edgePath(linkDrag.x1, linkDrag.y1, linkDrag.x2, linkDrag.y2));
      ...
      function renderGraph() {
        ...
        state.graphNodes.forEach(function (node) {
          ...
          title.textContent = node.title || (node.type === "goal" ? "" : "New item");
          ...
          title.addEventListener("input", function () {
            ...
            n.title = plainText(getRichHtml(title));
            queueSave();
          });
      ...
      function hitInputPort(ev) {
        ...
        var nodeEl = el.closest ? el.closest(".gnode") : null;
        if (nodeEl) return nodeEl.dataset.id;
        return null;
      }
