...
OUTPUT = ROOT / "projects" / "agro_brdf.html"
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .graph-svg .edge-group.is-hot .edge-x-g {
      opacity: 1;
      pointer-events: all;
    }
    .graph-svg .edge-hit {
    ...
    .gnode.selected {
      border-color: var(--accent);
      box-shadow: 0 0 0 2px rgba(31, 122, 108, 0.35), 0 4px 12px rgba(15, 28, 36, 0.08);
    }
    .selection-marquee {
      position: absolute;
    ...
    .gnode:hover .node-resize,
    .gnode:focus-within .node-resize {
      opacity: 1;
    }
    .port {
      position: absolute;
    ...
    .graph-stage.is-linking .gnode.compact-ports .port {
      opacity: 1;
      pointer-events: auto;
    }
    ...
    <section class="panel" id="pane-graph">
      ...
      <div class="graph-layout">
        <div class="graph-stage" id="graph-stage">
          <div class="graph-bg" id="graph-bg"></div>
        ...
        <aside class="graph-toolbar">
          ...
          <div class="hint">Drag empty area to multi-select. Drag selected together. Ctrl+C/V copy. Delete key removes selected tasks. Top ports link. Hover edge for X. Title: right-click Add link. Click linked text to open.</div>
        </aside>
      </div>
    </section>
  </div>
  ...
  <script>
    (function () {
      ...
      function sanitizeHtml(html) {
        ...
        box.querySelectorAll("*").forEach(function (el) {
          ...
          attrs.forEach(function (attr) {
            ...
            if (tag === "a") {
              if (name !== "href" && name !== "target" && name !== "rel" && name !== "contenteditable") {
                el.removeAttribute(attr.name);
              }
      ...
      function setNodeSelected(id, on) {
        if (on) selectedNodeIds[id] = true;
        else delete selectedNodeIds[id];
      }

      function selectedIdList() {
        return Object.keys(selectedNodeIds);
      }
      ...
      function applySelectionClasses() {
        var layer = document.getElementById("graph-nodes");
        if (!layer) return;
        Array.prototype.forEach.call(layer.querySelectorAll(".gnode"), function (el) {
          if (isNodeSelected(el.dataset.id)) el.classList.add("selected");
      ...
      function nodeRect(n) {
        ...
        return { x1: n.x, y1: n.y, x2: n.x + size.w, y2: n.y + h };
      }

      function ensureMarqueeEl() {
        var stage = document.getElementById("graph-stage");
        var el = document.getElementById("selection-marquee");
        if (!el) {
      ...
      function selectByMarquee() {
        if (!marquee) return;
        var box = {
          x1: Math.min(marquee.x0, marquee.x1),
          y1: Math.min(marquee.y0, marquee.y1),
          x2: Math.max(marquee.x0, marquee.x1),
          y2: Math.max(marquee.y0, marquee.y1)
        };
        if (!marquee.additive) clearNodeSelection();
        state.graphNodes.forEach(function (n) {
          if (rectsIntersect(box, nodeRect(n))) setNodeSelected(n.id, true);
        });
      ...
      function deepCloneNode(n) {
        return {
          id: n.id,
          type: n.type,
          goalIndex: n.goalIndex == null ? null : n.goalIndex,
          title: n.title || "",
          x: Number(n.x) || 0,
          y: Number(n.y) || 0,
          w: n.w == null ? null : n.w,
      ...
      function copySelectionToClipboard() {
        ...
        var nodes = state.graphNodes.filter(function (n) { return idSet[n.id]; }).map(deepCloneNode);
      ...
      function pasteClipboard(offset) {
        if (!clipboard || !clipboard.nodes || !clipboard.nodes.length) return false;
        var dx = offset == null ? GRID * 2 : offset;
        var dy = offset == null ? GRID * 2 : offset;
        var idMap = {};
        clearNodeSelection();
        clipboard.nodes.forEach(function (src) {
          var nid = uid("task");
          idMap[src.id] = nid;
          state.graphNodes.push({
            ...
            type: "task",
            goalIndex: null,
            title: src.title || "New item",
            x: src.x + dx,
            y: src.y + dy,
            w: src.w == null ? null : src.w,
            h: src.h == null ? null : src.h
          });
          setNodeSelected(nid, true);
        });
      ...
      function normalizeEnteredUrl(url) {
        var h = (url != null ? String(url) : "").trim();
        if (!h) return "";
        if (/^https?:\/\//i.test(h) || h.charAt(0) === "/" || h.charAt(0) === "#" || h.charAt(0) === "?") {
          return h;
        }
        if (h.indexOf("://") >= 0) return h;
        if (/^www\./i.test(h) || /^[\w.-]+\.[a-z]{2,}([/:?#]|$)/i.test(h)) {
          return "https://" + h;
        }
        return h;
      }

      function closestTitleAnchor(node, root) {
        var el = node;
      ...
      function findTitleAnchorFromEvent(ev, titleEl) {
        if (!ev || !titleEl) return null;
        if (ev.composedPath) {
          var path = ev.composedPath();
          var i;
          for (i = 0; i < path.length; i++) {
            var n = path[i];
            if (n && n.tagName && n.tagName.toLowerCase() === "a" && titleEl.contains(n)) return n;
          }
        }
        var t = ev.target;
        ...
        if (t && t.closest) {
          var a = t.closest("a");
          if (a && titleEl.contains(a)) return a;
        }
        return closestTitleAnchor(t, titleEl);
      }

      function captureTitleRange(titleEl) {
        if (!titleEl) return null;
        var sel = window.getSelection();
        if (sel && sel.rangeCount >= 1 && !sel.isCollapsed) {
          var selRange = sel.getRangeAt(0);
          var anc = selRange.commonAncestorContainer;
          if (titleEl === anc || titleEl.contains(anc)) {
            return selRange.cloneRange();
          }
        }
        var all = document.createRange();
        all.selectNodeContents(titleEl);
        return all;
      }

      function markTitleAnchors(titleEl) {
        if (!titleEl) return;
      ...
      function persistTitleHtml(titleEl) {
        titleEl.innerHTML = sanitizeHtml(titleEl.innerHTML);
        markTitleAnchors(titleEl);
      ...
      function wrapRangeWithLink(titleEl, range, url) {
        if (!titleEl || !range || !url) return false;
        var anc = range.commonAncestorContainer;
        if (titleEl !== anc && !titleEl.contains(anc)) return false;

        var startA = closestTitleAnchor(range.startContainer, titleEl);
        var endA = closestTitleAnchor(range.endContainer, titleEl);
        if (startA && startA === endA) {
          startA.setAttribute("href", url);
          startA.href = url;
          startA.setAttribute("target", "_blank");
          startA.setAttribute("rel", "noopener noreferrer");
          startA.setAttribute("contenteditable", "false");
          return true;
        }
      ...
      function showTitleLinkMenu(clientX, clientY, titleEl, rangeOpt, currentHref) {
        hideTitleCtxMenu();
        var savedRange = rangeOpt ? rangeOpt.cloneRange() : captureTitleRange(titleEl);
        ...
        var btn = document.createElement("button");
        btn.type = "button";
        btn.textContent = currentHref ? "Edit link" : "Add link";
        btn.addEventListener("click", function (ev) {
          ev.preventDefault();
          ev.stopPropagation();
          hideTitleCtxMenu();
          var url = window.prompt("URL", currentHref || "https://");
          if (!url) return;
          url = normalizeEnteredUrl(url);
          if (!isSafeHref(url)) {
            window.alert("Only http(s) or relative URLs are allowed.");
            return;
          }
          if (!wrapRangeWithLink(titleEl, savedRange, url)) return;
          persistTitleHtml(titleEl);
        });
      ...
      function renderGraph() {
        ...
        state.graphNodes.forEach(function (node) {
          ...
          if (node.type === "task") {
            ...
            markTitleAnchors(title);
          } else {
            title.textContent = node.title || "";
          }
          title.addEventListener("mousedown", function (ev) {
            var aDown = findTitleAnchorFromEvent(ev, title);
            if (aDown && aDown.getAttribute("href")) {
              ev.stopPropagation();
              ev.preventDefault();
              return;
            }
          ...
          title.addEventListener("click", function (ev) {
            var a = findTitleAnchorFromEvent(ev, title);
            if (a && a.getAttribute("href") && isSafeHref(a.getAttribute("href"))) {
              ev.preventDefault();
              ev.stopPropagation();
          ...
          title.addEventListener("contextmenu", function (ev) {
            ...
            ev.preventDefault();
            ev.stopPropagation();
            var aCtx = findTitleAnchorFromEvent(ev, title);
            var href = aCtx && aCtx.getAttribute("href") ? aCtx.getAttribute("href") : "";
            showTitleLinkMenu(ev.clientX, ev.clientY, title, captureTitleRange(title), href);
          });
      ...
      function onNodeMouseDown(ev) {
        ...
        if (ev.shiftKey) {
          setNodeSelected(id, !isNodeSelected(id));
          applySelectionClasses();
          return;
        }
        ...
        var origins = {};
        selectedIdList().forEach(function (sid) {
          var nn = findNode(sid);
          if (nn) origins[sid] = { x: Number(nn.x) || 0, y: Number(nn.y) || 0 };
        });
        groupDrag = {
          startX: pt.x,
          startY: pt.y,
        ...
        selectedIdList().forEach(function (sid) {
          var nel = document.querySelector('.gnode[data-id="' + sid + '"]');
          if (nel) nel.classList.add("dragging");
        });
        suppressClick = false;
      }

      function isMarqueeStartTarget(t) {
        if (!t || !t.id && !(t.classList)) return false;
        if (t.id === "graph-stage" || t.id === "graph-bg" || t.id === "graph-svg" || t.id === "graph-nodes") return true;
        if (t.classList && (t.classList.contains("graph-stage") || t.classList.contains("graph-bg") || t.classList.contains("graph-svg") || t.classList.contains("graph-nodes"))) return true;
        return false;
      }
      ...
      function onPortMouseDown(ev) {
        ...
        selectedEdgeId = null;
        document.getElementById("graph-stage").classList.add("is-linking");
        renderEdges();
      }
      ...
      function onMouseMove(ev) {
        ...
        if (groupDrag) {
          var gpt = stagePoint(ev);
          var gdx = gpt.x - groupDrag.startX;
          var gdy = gpt.y - groupDrag.startY;
          if (Math.abs(gdx) + Math.abs(gdy) > 3) {
            groupDrag.moved = true;
            suppressClick = true;
          }
          Object.keys(groupDrag.origins).forEach(function (sid) {
            var n = findNode(sid);
            var o = groupDrag.origins[sid];
            if (!n || !o) return;
            var nx = Math.max(0, snapToGrid(o.x + gdx));
            var ny = Math.max(0, snapToGrid(o.y + gdy));
            n.x = nx;
            n.y = ny;
            var nel = document.querySelector('.gnode[data-id="' + sid + '"]');
            if (nel) {
              nel.style.left = nx + "px";
              nel.style.top = ny + "px";
            }
          });
          renderEdges();
      ...
      function onMouseUp(ev) {
        ...
        if (groupDrag) {
          ...
          if (groupDrag.moved) suppressClick = true;
          groupDrag = null;
          if (!applyingRemote) queueSave();
          return;
        }
        ...
        if (linkDrag) {
          ...
          linkDrag = null;
          document.getElementById("graph-stage").classList.remove("is-linking");
          renderEdges();
          if (toId && toId !== fromId) {
      ...
      function wireButtons() {
        ...
        var stageEl = document.getElementById("graph-stage");
        ...
        document.addEventListener("keydown", function (ev) {
          ...
          if ((ev.ctrlKey || ev.metaKey) && (ev.key === "c" || ev.key === "C")) {
            if (copySelectionToClipboard()) ev.preventDefault();
            return;
          }
          ...
          ids.forEach(function (id) {
            var n = findNode(id);
            if (!n || n.type === "goal") return;
            state.graphNodes = state.graphNodes.filter(function (nn) { return nn.id !== id; });
            state.edges = state.edges.filter(function (e) {
              return e.from !== id && e.to !== id;
            });
            delete selectedNodeIds[id];
          });
          renderGraph();
          queueSave();
        });
